import React, { useState,useEffect} from 'react'
import './Pages.css'
import {Match} from './Match'
import styled from 'styled-components'
import axios from 'axios'
import './Pages.css'
const NomeIdade = styled.div`
    position: absolute;
    top:450px;
    margin-left: 2%;
    margin-right: 40%;
    color:black;
`
const Descricao=styled.h4`
    margin-top: 0%;
`
const  SeparaCoracao = styled.div`
    display: flex;
    justify-content: space-evenly;
`
const Coracao = styled.img`
    border-radius: 100%;
    width: 100px;
    height: 80px;
    border: 1px solid green;
    margin-top:15% ;
`

const Imagem = styled.img`
    width: 10%;
    display: flex;
`
const AstroMatch = styled.img`
    width: 40%;
`
const AstroCentralizar = styled.div`
    justify-self: center;
    align-items: center;
`
const ImagemTinder = styled.img`
    width: 340px;
    height: 378px;
    margin-top: 2%;
   
`
const Body = styled.div`
    width: 390px;
    height: 588px;
    border: 1px solid black;
    margin-top:5%;
    margin-right: 50%;
    margin-left: 30%;
     display: flex;
    flex-direction: column;
    /*display: grid;
    grid-template-rows:10% 2fr 10% ;*/
   

`
const IconeAstro = styled.div`
    display: flex;
    justify-content: space-between;
    padding: 1%;
    margin-top: 0%;
    border-bottom: 1px solid black;
`
export const Pages = () => {
    const [imagem, setImagem] = useState('')
    const [nome, setName] = useState('')
    const [idade, setIdade] = useState(0)
    const [descricao, setDescricao] = useState('')
    const [iden,setIden] = useState('')
    const [escolha,setEscolha] = useState()
    const [tentativa,setTentativa] = useState(true)
    const [tela,setTela]=useState('pages')
    useEffect(()=>{
        getProfile()
    },[])
    const trocaTela = () =>{
        setTela('match')
    }
     const onClickPage = () => {
        setTela('pages')
    }
    const getProfile = () => {
        const url = 'https://us-central1-missao-newton.cloudfunctions.net/astroMatch/matheus-molina/person'
        axios.get(url)
            .then((res) => {
                setImagem(res.data.profile.photo)
                setName(res.data.profile.name)
                setIdade(res.data.profile.age)
                setDescricao(res.data.profile.bio)
                setIden(res.data.profile.id)
                
            }).catch((err) => {
                console.log(err)
            })
    }
    const choosePerson = (tr) => {
        console.log('t',tr)
        const algo =tr
        const url = 'https://us-central1-missao-newton.cloudfunctions.net/astroMatch/matheus-molina/choose-person'
        const headers = {
            headers: {
                Authorization: "matheus-molina"
            }
        }
        const body = {
            id:iden,
            choice:algo
        }
        console.log('body',body)
        axios.post(url,body,headers)
            .then((res)=>{
                getProfile()
            }).catch((err) => {
                console.log(err.data)
            })
    }
    const clear = () => {
        const url ='https://us-central1-missao-newton.cloudfunctions.net/astroMatch/matheus-molina/clear'
        const headers = {
            headers: {
                Authorization: "matheus-molina"
            }
        }
        axios.put(url,headers)
        .then(()=>{
            getProfile()
        }).catch((err) => {
            console.log(err.data)
        })

    } 
    
    return (
        <div>
            <div className='body'>
            <IconeAstro>
                <AstroCentralizar>
                    <AstroMatch onClick={onClickPage} src='data:image/png;base64,' />

                </AstroCentralizar>
                <Imagem src='https://img.icons8.com/ios/500/--tinder.png' onClick={trocaTela}/>
            </IconeAstro>
            {tela==='match'? <Match/>: tela==='pages'?
            <div>
           
           
                <div className='separar'>
                <ImagemTinder src={imagem} />
                </div>
                
                <div className='nomeIdade'>
                    <h2>{nome}, {idade}</h2>  
                    <Descricao>{descricao}</Descricao>   
                </div>
             
           
            <div className='footer'>
            <Coracao className='core' onClick={()=>choosePerson(false)} src='https://pbs.twimg.com/profile_images/534074996562227200/OR7cp94I.png'/>
            <Coracao className='core' onClick={()=>choosePerson(true)} src='https://www.kindpng.com/picc/m/132-1323936_tinder-heart-and-x-hd-png-download.png'/>
            </div>
        </div>: <div></div>
            }
        
        </div>
            <button onClick={clear}>limpar</button>

        </div>
    )
}
